...
const { makeDate, numifyBail } = require('../../utils/dataShapers');


const scrapeDetention = bookingNumber => {
...
const makeDetentionObject = html => {
  const rawDate = html('label[for="BookingDateTime"]').parent().next().text();
  const timelessDate = rawDate.slice(0, 10);
  const formattedDate = makeDate(timelessDate);
  console.log(rawDate);
  console.log(timelessDate);
  console.log(new Date(formattedDate));
  return ({
    county: 'Multnomah',
    dateAdded: new Date(makeTimeless()),
